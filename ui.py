...
from test_email_attach import send_mail_attach
...
class UIman():
    ...
    def ele_mgr_forserch(self, timeout, method):
        try:
            return WebDriverWait(self.driver, timeout, poll_frequency=0.2).until(method)
        except TimeoutException:
            print('出现了TimeoutException')
            return None
        except WebDriverException:
    ...
    def searchIdMap(self,id,idmap):
        for i, v in idmap.items():
            try:
                ...
                exc_type, exc_value, exc_traceback = sys.exc_info()
                traceback.print_exception(exc_type, exc_value, exc_traceback)
                print("searchIdMap 过程中出现了异常")
                print(id)
                print(self.driver)
                raise WebDriverException('testError')
        return None
    ...
    def findidAndIdClick(self, id, id2, timeout=TIMEOUT):
        while timeout > 0:
            try:
                ...
                exc_type, exc_value, exc_traceback = sys.exc_info()
                traceback.print_exception(exc_type, exc_value, exc_traceback)
                print("findidAndIdClick 过程中出现了异常")
                print(id)
                print(self.driver)
                raise WebDriverException('testError')
        return None
    ...
    def find_text_in_id(self, id, text1, id2, timeout=TIMEOUT):
        ...
        while timeout > 0:
            try:
                ...
                raise WebDriverException('testError')
        return None
    def find_contentdesc_in_id(self, id, text1, timeout=TIMEOUT):
        print("开始找元素")
        while timeout > 0:
            try:
                time.sleep(TIMEGAP)
                tt1 = time.time()
                el = self.driver.find_element_by_id(id)
                if el.get_attribute('content-desc') == text1:
                    return 'true'
            except NoSuchElementException:
                t1 = time.time() - tt1
                if t1 > timeout:
                ...
                exc_type, exc_value, exc_traceback = sys.exc_info()
                traceback.print_exception(exc_type, exc_value, exc_traceback)
                print("find_contentdesc_in_id 过程中出现了异常")
                print(id)
                print(self.driver)
                raise WebDriverException('testError')
        return None
    ...
    def findname(self, name, timeout=TIMEOUT):
        while timeout > 0:
            try:
                time.sleep(TIMEGAP)
    ...
    def findids(self, id, timeout=TIMEOUT):
        while timeout > 0:
            try:
                time.sleep(TIMEGAP)
    ...
    def findxpath(self, xpath, timeout=TIMEOUT):
        while timeout > 0:
            try:
                ...
                exc_type, exc_value, exc_traceback = sys.exc_info()
                traceback.print_exception(exc_type, exc_value, exc_traceback)
                print("findxpath 过程中出现了异常")
                print(self.driver)
                raise WebDriverException('testError')
        return None

    def findxpath_for_search(self, xpath, timeout=TIMEOUT):
        while timeout > 0:
            try:
                # time.sleep(0.2)
                el = self.driver.find_element_by_xpath(xpath)
    ...
    def screenShot(self, name):
        ...
        # folder = os.path.abspath(os.path.dirname(__file__)) + '\\screenshots\\'
        ...
        if not self.driver.get_screenshot_as_file(folder + name + time_str + ".png"):
            ...
            return 'false '
        send_mail_attach(name, str(folder + name + time_str + ".png"), name+ time_str + ".png")
